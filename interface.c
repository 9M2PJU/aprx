	...
	while (readconfigline(cf) != NULL) {
		...
		if (strcmp(name,"ax25-device") == 0) {
		  ...
		  ++defined_subinterface_count;
                  ...
                  int bool_val;
		  if (!config_parse_boolean(param1, &bool_val)) {
		    printf("%s:%d ERROR: Bad TX-OK parameter value -- not a recognized boolean: %s\n",
			   cf->name, cf->linenum, param1);
		    have_fault = 1;
		    continue;
                  ...
                  aif->tx_ok = bool_val;
		  if (bool_val && aif->callsign) {
		    if (!validate_callsign_input(aif->callsign,bool_val)) {  // Transmitters REQUIRE valid AX.25 address
